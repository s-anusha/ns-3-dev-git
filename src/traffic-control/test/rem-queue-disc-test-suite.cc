...
private:
  ...
  */
  void RunRemTest (QueueSizeUnit/*StringValue*/ mode);
};
...
void
RemQueueDiscTestCase::RunRemTest (QueueSizeUnit/*StringValue*/ mode)
{
  uint32_t pktSize = 0;
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue () /*GetQueueSize ()*/, 7 * modeSize, "There should be seven packets in there");
  NS_TEST_EXPECT_MSG_EQ (item->GetPacket ()->GetUid (), p1->GetUid (), "was this the first packet ?");

  item = queue->Dequeue ();
  NS_TEST_EXPECT_MSG_EQ ((item != 0), true, "I want to remove the second packet");
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue () /*GetQueueSize ()*/, 6 * modeSize, "There should be six packet in there");
  NS_TEST_EXPECT_MSG_EQ (item->GetPacket ()->GetUid (), p2->GetUid (), "Was this the second packet ?");

  item = queue->Dequeue ();
  NS_TEST_EXPECT_MSG_EQ ((item != 0), true, "I want to remove the third packet");
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue () /*GetQueueSize ()*/, 5 * modeSize, "There should be five packets in there");
  ...
  Simulator::Run ();
  st = StaticCast<RemQueueDisc> (queue)->GetStats ();
  uint32_t test4 = st.GetNDroppedPackets (RemQueueDisc::UNFORCED_DROP);
  ...
  st = StaticCast<RemQueueDisc> (queue)->GetStats ();
  uint32_t test5 = st.GetNDroppedPackets (RemQueueDisc::UNFORCED_DROP);
